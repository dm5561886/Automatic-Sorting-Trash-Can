  ...
  <?
	for($month=1;$month<=12;$month++)
	{
		switch($month)
		{
			...
			{	
				$sql_query ="SELECT * FROM `trash_identify` WHERE `t_month`='".$month."' and `t_date`='".$date."' and `id_serial`='0'";
				$result_query=mysqli_query($db_link,$sql_query);
				$rows_query=mysqli_fetch_row($result_query);
				$sql="SELECT COUNT(*) FROM `trash_identify` WHERE `t_month`='".$month."' and `t_date`='".$date."' and `id_serial`='0'";
